...
router.post('/createquote',requirelogin,(req,res)=>{
    const {quote}=req.body
    console.log("req.user",req.user)
    if(!quote){
        return res.status(422).json({error:"Please add all the necessary fields"})
    }
    const quoteBody=new quotePost({
        quote,
        postedBy:req.user
    })
    quoteBody.save().then(result=>{
        res.json({quoteBody:result})
    })
    .catch(err=>{
        console.log(err)
    })
